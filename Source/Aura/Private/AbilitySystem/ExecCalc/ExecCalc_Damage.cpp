...
void UExecCalc_Damage::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	if (FMath::RandRange(1, 100) < BlockChance)
	{
		Damage /= 2.f;
	}
	...
	const float EffectiveArmorCoefficient = EffectiveArmorCurve->Eval(TargetCombatInterface->GetLevel());
	...
	EffectiveArmor = FMath::Min<float>(EffectiveArmor, 100.f);
	Damage *= ( 100 - EffectiveArmor * EffectiveArmorCoefficient ) / 100.f;
	
	Damage = FMath::Max<float>(Damage, 1.f);
	
	// 获取暴击，暴击抗性会无视部分暴击，爆伤为暴击增加伤害系数
